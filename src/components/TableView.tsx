...
import { Edit, Trash2, Eye, MoreHorizontal, Filter, Grid, List, AlertTriangle, CheckCircle, Clock, GitBranch, Database, Shield, UserCheck } from 'lucide-react';
      ...
      <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border overflow-hidden`}>
        <div className={`px-6 py-4 ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} border-b`}>
          <div className="flex items-center space-x-4">
            <div className={`p-3 ${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'} rounded-lg`}>
              <GitBranch className={`h-6 w-6 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`} />
            </div>
            <div>
        ...
        <div className="p-6">
          <div className="space-y-4">
            {[
              { action: 'Record Updated', user: 'John Doe', time: '2 hours ago', details: 'Modified customer email address', approved: true },
              { action: 'Bulk Import', user: 'System', time: '1 day ago', details: 'Imported 45 new records from CSV', approved: true },
              { action: 'Record Proposed', user: 'Sarah User', time: '3 hours ago', details: 'New customer entry awaiting approval', approved: false },
            ].map((entry, index) => (
              <div key={index} className={`flex items-center space-x-4 p-4 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg border ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                <div className={`p-2 rounded-lg ${entry.approved ? 'bg-green-100' : 'bg-orange-100'}`}>
                  {entry.approved ? (
                ...
                <div className="flex-1">
                  ...
                  {!entry.approved && (
                    <div className="text-xs text-orange-600 font-medium mt-1">Awaiting Admin Approval</div>
                  )}
                </div>
                <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} text-right`}>
    ...
    return (
      <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border overflow-hidden`}>
        <div className={`px-6 py-4 ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} border-b`}>
          <div className="flex items-center space-x-4">
            <div className={`p-3 ${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'} rounded-lg`}>
              <Database className={`h-6 w-6 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`} />
            </div>
            <div>
              <h3 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                {table.name} Lineage
            ...
            <div className={`${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} rounded-lg p-4 border`}>
              ...
              <div className="space-y-2">
                {['CRM Database', 'ERP System', 'External API'].map((source, index) => (
                  <div key={index} className={`flex items-center space-x-2 p-2 ${isDarkMode ? 'bg-gray-600' : 'bg-white'} rounded border ${isDarkMode ? 'border-gray-500' : 'border-gray-200'}`}>
                    <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{source}</span>
                  </div>
                ))}
              </div>
            </div>
            
            <div className={`${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} rounded-lg p-4 border flex items-center justify-center`}>
              <div className="text-center">
                <div className={`p-4 ${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'} rounded-lg mx-auto mb-3`}>
                  <Icon className={`h-8 w-8 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`} />
                </div>
              ...
              <div className="space-y-2">
                {['Analytics Dashboard', 'Reporting Engine', 'ML Pipeline'].map((consumer, index) => (
                  <div key={index} className={`flex items-center space-x-2 p-2 ${isDarkMode ? 'bg-gray-600' : 'bg-white'} rounded border ${isDarkMode ? 'border-gray-500' : 'border-gray-200'}`}>
                    ...
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{consumer}</span>
                  </div>
                ))}
              </div>
            </div>
